        ...
        {blog.slice(0, 3).map(item => 
          <div className="shadow-xl rounded-lg" key={item.id}>
            ...
            <div className="p-4">
              ...
              <Link className="text-xs underline text-blue-500" to={`/singleBlogCard/${item._id}`}>see more</Link>
              </p>
            <Link to={`/singleBlogCard/${item._id}`}>
              <button className="details rounded px-3 py-1 text-white font-semibold bg-red-300 mt-3">details</button>
            </Link>
            </div>
          </div>
        )}
