...
const Cover: React.FC<CoverProps> = () => {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 lg:mx-auto gap-4 mx-4 px-5">
            <div className="text-left mx-auto py-9">
                ...
                <h2 className="text-5xl font-semibold text-yellow-500 font-abc py-4">knowledge</h2>
                <p className="tracking-normal my-4">
                    Lorem ipsum, dolor sit amet consectetur adipisicing elit.<br />
                    Quia voluptates libero exercitationem, doloribus commodi hic<br />
                    inventore soluta nemo quo deserunt laborum ut rerum fugiat<br />
                    fuga aliquid. Suscipit placeat pariatur consequuntur!
                </p>
                <button className="btn btn-accent my-6">Accent</button>
            </div>

            <div>
                <Swiper navigation={true} modules={[Navigation]} className="w-full lg:h-96 md:h-auto">
                    <Swiper navigation={true} modules={[Navigation]} className="w-full">
                        {images.map((imagePath, index) => (
                            <SwiperSlide key={index}>
                                <img src={imagePath} alt={`Slide ${index + 1}`} className='lg:h-96 md:h-10' />
                            </SwiperSlide>
                        ))}
                    </Swiper>
