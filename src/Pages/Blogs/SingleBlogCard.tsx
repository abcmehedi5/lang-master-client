...
const SingleBlogCard = () => {
  ...
  const ownBlog = blog.filter(own => own.email === data.email);
  console.log(ownBlog)
  ...
  return (
    <div
      ...
      // }}
      className=" w-10/12 py-4 my-8 mx-auto flex gap-6 flex-col-reverse md:flex-row"
    >

      <div className="md:w-[70%] sticky top-0 h-full">
        <img className="w-full h-[550px] object-cover rounded-lg" src={data?.image} alt="post image" />
        <p className="text-gray-400 my-3">{data?.uploadedtime}</p>
        <h1 className="text-3xl mb-2 font-semibold capitalize">{data?.title}</h1>
...
{/* -------------------------------------------------- */}
        <hr className="my-4"/>
         ...
         <div className="card-actions justify-between m-4 font-semibold text-2xl">
                      {/* Like  */}
                      <Like postId={data._id} like={data.like} />
                      {/* Comment */}
                      {/* <button
      ...
      <div className="md:w-[30%] mx-auto text-center">
        ...
        <img className="w-[100px] mx-auto h-[100px] object-cover" src={data?.authorImage} alt="" />
        <div className="text-start">
        <h2 className="text-2xl my-3 font-semibold">{data?.name}</h2>
        <p className="flex gap-1 text-lg"><AiTwotoneMail className="text-3xl" /> : {data?.email}</p>
        </div>
        </div>
        <hr className="my-4"/>
        {/* ----------------------- */}
        <h4 className="text-2xl mb-2 font-semibold">More Content</h4>
        {ownBlog.map(blog => <div key={blog._id}>
