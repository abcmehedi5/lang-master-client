...
const AllBuyBook: React.FC = () => {
  ...
  const confirmDelete = async () => {
    if (itemToDeleteId !== null) {
      try {
        await axios.delete(
          `http://localhost:5000/books/book/${itemToDeleteId}`
        );
  ...
  const columns = [
    {
      ...
      renderCell: (params: any) => (
        <img src={params.value} alt=" Book" className="rounded-full" />
      ),
    },
    ...
    { field: "status", headerName: "status", width: 75 },
    {
      ...
      renderCell: (params: any) => (
        <button onClick={() => handleDelete(params.row._id)}>
          <AiTwotoneDelete className="text-center text-xl text-red-500" />
        </button>
      ),
    },
  ];
